...
func time_irrigation():
	if waterStart>0 and time>waterStart+1.5:
		remove_child(currentWatering)
		
	if !bloomCondition and notwithered and waterStart>0 and time>waterStart+5:
		bloomCondition=true
		bloom()
		
	if plantstart>0 and notwithered and time>plantstart+5 and timesWatered==0:
		wither()
		state=-1
...
func _on_Area2D_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.pressed:
		if get_tree().get_root().get_node("Game").get_node("Board").isPrimitive:
			advance()
		else:
			get_tree().get_root().get_node("Game").get_node("Board").isPrimitive = true
